...
st.set_page_config(
    page_title="Proyecto Integrador UCG - Dashboard de Analítica",
    page_icon="🦐",
    layout="wide",
...
# Título y Sidebar de la Aplicación
st.title("🎓 Proyecto Integrador de Ciencia de Datos - UCG")

st.sidebar.markdown("<div style='text-align: center;'><h2 style='color:#38bdf8;margin-bottom:0;'>Antigravity Analytics</h2><p style='color:#94a3b8;font-size:0.8rem;'>Acuicultura y Finanzas</p></div>", unsafe_allow_html=True)
...
modulo = st.sidebar.selectbox(
    ...
    ["Módulo 1: Carga de Datos", "Módulo 2: Simulación de Créditos", "Módulo 3: Sector Camaronero (Proyecto Final)"]
)

# ==============================================================================
# MÓDULO 1: CARGA DE DATOS GENERAL
# ==============================================================================
if modulo == "Módulo 1: Carga de Datos":
    st.header("📂 Carga General de Datos")
    st.write("Cargue archivos en formato CSV para explorar su estructura y contenido de forma interactiva.")
    
    uploaded_files = st.file_uploader(
        "Subir Archivos CSV", accept_multiple_files=True, type="csv"
    )
    ...
    if uploaded_files:
        for uploaded_file in uploaded_files:
            ...
            try:
                ...
                with col2:
                    st.markdown("**Primeros Registros (Vista Previa):**")
                    st.dataframe(df.head(10), use_container_width=True)
            except Exception as e:
                st.error(f"Error al leer el archivo {uploaded_file.name}: {str(e)}")
    else:
        st.info("Suba uno o más archivos CSV en el recuadro superior para comenzar a visualizarlos.")

# ==============================================================================
# MÓDULO 2: SIMULACIÓN DE CRÉDITOS
# ==============================================================================
elif modulo == "Módulo 2: Simulación de Créditos":
    st.header("🏦 Simulador Financiero de Préstamos")
    st.write("Calcule las cuotas periódicas de préstamos utilizando fórmulas financieras estructuradas.")
    
    tab1, tab2, tab3 = st.tabs(["💰 Ingreso de Parámetros", "📋 Cálculo de Cuotas", "🔍 Resumen y Análisis"])

    with tab1:
        st.subheader("Configuración del Préstamo")
        st.write("Defina el monto, la tasa de interés y el plazo del préstamo.")
        ...
        with col1:
            monto = st.number_input(
                "Monto del Préstamo ($)", 
                min_value=0, max_value=1000000, value=10000, step=500,
                help="El monto de capital solicitado."
            )
        with col2:
            interes = st.number_input(
                "Tasa de Interés Anual (Decimal)", 
                min_value=0.0, max_value=1.0, value=0.12, step=0.01,
                help="Tasa de interés expresada en decimal. Ejemplo: 0.12 para el 12%."
            )
            anios = st.number_input(
                "Plazo del Préstamo (Años)", 
                min_value=1, max_value=30, value=5, step=1,
                help="Número de años para pagar el préstamo."
            )
        with col3:
            numero_pagos = st.number_input(
                "Pagos por Año (Frecuencia)", 
                min_value=1, max_value=365, value=12, step=1,
                help="Frecuencia de pagos anuales. 12 = Mensual, 4 = Trimestral, 1 = Anual."
            )

    with tab2:
        st.subheader("Cálculo de la Cuota")
        
        # Calcular cuota con función modular
        cuota = lf.cuota_prestamo(monto, interes, anios, numero_pagos)
        
        col_c1, col_c2 = st.columns(2)
        with col_c1:
            st.markdown("<div style='background-color:#1e293b;border-radius:12px;padding:25px;border:1px solid #334155;text-align:center;'>", unsafe_allow_html=True)
            st.write(f"Su cuota periódica estimada es de:")
            st.markdown(f"<h1 style='color:#10b981;font-size:3rem;margin:10px 0;'>${cuota:,.2f}</h1>", unsafe_allow_html=True)
            frecuencia_txt = "Mensual" if numero_pagos == 12 else ("Trimestral" if numero_pagos == 4 else ("Anual" if numero_pagos == 1 else f"Cada {round(365/numero_pagos)} días"))
            st.write(f"Frecuencia de pago: **{frecuencia_txt}**")
            st.markdown("</div>", unsafe_allow_html=True)
            
        with col_c2:
            st.markdown("**Detalles de la Simulación:**")
            pago_total = cuota * anios * numero_pagos
            interes_total = pago_total - monto
            st.write(f"- **Monto Principal Solicitado:** ${monto:,.2f}")
            st.write(f"- **Tasa de Interés Nominal:** {interes*100}% anual")
            st.write(f"- **Total de Cuotas a Pagar:** {int(anios * numero_pagos)}")
            st.write(f"- **Pago Total Acumulado:** ${pago_total:,.2f}")
            st.write(f"- **Interés Total Pagado:** ${interes_total:,.2f}")

    with tab3:
        st.subheader("Estructura de Amortización Proyectada")
        st.write("Representación visual del costo total del préstamo.")
        
        # Gráfica interactiva de estructura de pagos
        fig_prest = go.Figure(data=[go.Pie(
            labels=['Capital Principal', 'Intereses Totales'], 
            values=[monto, (cuota * anios * numero_pagos) - monto],
            hole=.4,
            marker=dict(colors=['#38bdf8', '#ef4444'])
        )])
        fig_prest.update_layout(
            title="Estructura del Desembolso Total",
            template='plotly_dark'
        )
        st.plotly_chart(fig_prest, use_container_width=True)

# ==============================================================================
# MÓDULO 3: PROYECTO FINAL - SECTOR CAMARONERO
# ==============================================================================
else:
    st.header("🦐 Panel Analítico del Sector Camaronero")
    st.write("Plataforma interactiva para el monitoreo de crecimiento, alimentación y rentabilidad en piscinas acuícolas.")
    
    # Selector de origen de datos en sidebar
